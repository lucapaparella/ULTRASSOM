...
convolve_xp = None      # Será definida depois (versão CPU ou GPU da convolve1d)
...
if USE_CUDA:
    try:
        ...
        if cp.cuda.runtime.getDeviceCount() > 0:
            ...
            convolve_xp = c_convolve     # Versão da transformada de convolve1d que roda na GPU
...
for acq in imagens:
    ...
    # Calcula o canal Q pela transformada de Hilbert
    qdata = xp.imag(hilbert_xp(idata, axis=-1))

    # Converte ângulos do arquivo (em graus) para radianos
    angles = xp.array(arquivo["angles"]) * np.pi / 180

    # Frequência central (fc)
    fc = xp.array(arquivo["modulation_frequency"]).item()

    # Frequência de amostragem do canal de RF
    fs = xp.array(arquivo["channel_data_sampling_frequency"]).item()

    # Velocidade do som usada na reconstrução
    c = sound_speed

    # time_zero vem do arquivo; multiplicamos por -1 para alinhar os sinais
    time_zero = -1 * xp.array(arquivo["channel_data_t0"], dtype="float32")

    # Garante que time_zero tenha o mesmo tamanho de angles
    if time_zero.size == 1:
        time_zero = xp.ones_like(angles) * time_zero

    # Demodulação em banda-base será feita usando fc
    fdemod = fc

    # Geometria do transdutor LA533
    pitch = 0.245e-3            # espaçamento entre elementos
    nelems = idata.shape[1]     # número de elementos
    xpos = xp.arange(nelems) * pitch
    xpos -= xp.mean(xpos)       # centraliza em x = 0
    ele_pos = xp.stack([xpos, 0 * xpos, 0 * xpos], axis=1)

    # Demodulação: converte o sinal para banda-base
    data = idata + 1j * qdata
    phase = xp.reshape(xp.arange(idata.shape[2], dtype="float"), (1, 1, -1))
    phase *= fdemod / fs
    data *= xp.exp(-2j * xp.pi * phase)

    # Fator de redução da taxa (downsample)
    dsfactor = int(xp.floor(fs / fc))

    # Filtro passa-baixas simples (média móvel)
    kernel = xp.ones((1, 1, dsfactor), dtype="float") / dsfactor

    # Convolução do sinal demodulado com o filtro
    data = convolve_xp(data, kernel, mode="constant")

    # Faz o downsample
    data = data[:, :, ::dsfactor]
    fs /= dsfactor   # atualiza a nova frequência de amostragem

    # Separa novamente I e Q
    idata = xp.real(data)
    qdata = xp.imag(data)

 
    # Limites laterais (x) e profundidade (z) da imagem
    ...
    def complex_rotate(I, Q, theta):
        """
        Rotaciona um sinal complexo (I + jQ) por um ângulo 'theta'.
        É o equivalente a multiplicar o sinal por exp(j*theta).

        Ir = parte real após rotação
        Qr = parte imaginária após rotação
        """
