...
for acq in imagens:
    ...
    # O dataset não possui os ângulos → criamos manualmente:
    # ângulos de -16° a +16°, um para cada transmissão
    angles = xp.linspace(-16, 16, idata.shape[0]) * xp.pi / 180

    # Frequência central, frequência de amostragem e velocidade do som
    ...
    # time_zero vem do arquivo; multiplicamos por -1 para alinhar com o modelo
    # [0] remove a dimensão extra
    time_zero = -1 * xp.array(arquivo["start_time"], dtype="float32")[0]

    # Posições dos elementos: arquivo tem (3,128), .T vira (128,3)
    ele_pos = xp.array(arquivo["element_positions"], dtype="float32").T

    # Centraliza todos os elementos em torno de x = 0
    ele_pos[:, 0] -= xp.mean(ele_pos[:, 0])

    # Ajuste de time_zero para cada ângulo devido à inclinação da onda plana
    ...
    # Limites laterais do transdutor e profundidade inicial
    xlims = xp.array([ele_pos[0, 0], ele_pos[-1, 0]])
    zlims = [10e-3, 60e-3]

    # Ajuste especial para imagens a partir da aquisição 13
    if acq >= 13:
        zlims = [10e-3, 50e-3]
