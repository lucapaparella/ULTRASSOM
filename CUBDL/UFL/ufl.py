...
for acq in imagens:
    ...
    print(f"time_zero => {time_zero.size}")
    ...
    # kernel = xp.ravel(kernel).astype(data.dtype)
    # print(f"kernel 1D => {kernel.shape}")
    # data = convolve_xp(data, kernel, axis=-1, mode="constant", cval=0.0)
    ...
    idata = xp.real(data)
    qdata = xp.imag(data)
    print(f"idata demodulado => {idata.shape}")
    ...
    # xlims_1 = (ele_pos[0, 0].item(), ele_pos[-1, 0].item())
    print(f"xlims => {xlims}")
    zlims = [10e-3, 50e-3]
    # zlims = (5e-3, 55e-3)
    print(f"zlims => {zlims}")
