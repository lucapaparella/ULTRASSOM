...
for acq in imagens:
    ...
    dataset = "EUT{:03d}".format(acq) + ".hdf5"
    ...
    # Carrega o canal I do sinal bruto
    idata = xp.array(arquivo["channel_data"], dtype="float32")

    # Calcula o canal Q usando a transformada de Hilbert no tempo
    qdata = xp.imag(hilbert_xp(idata, axis=-1))

    # Carrega os ângulos de transmissão (primeira coluna)
    angles = xp.array(arquivo["transmit_direction"])[:, 0]
    ...
    # start_time inicial do dataset (usaremos como base)
    time_zero = xp.array(arquivo["start_time"], dtype="float32")[0]

    # Não estamos usando demodulação
    fdemod = 0

    # Carrega as posições dos elementos e centraliza no eixo x
    ele_pos = xp.array(arquivo["element_positions"], dtype="float32").T
    ele_pos[:, 0] -= xp.mean(ele_pos[:, 0])

    # Ajuste de time_zero para cada ângulo
    for i, a in enumerate(angles):
        time_zero[i] = ele_pos[-1, 0] * xp.abs(xp.sin(a)) / c

    # Pequeno ajuste adicional no time_zero
    time_zero += 10 / fc
