...
for acq in imagens:
    ...
    # ------------------------------------------------------------
    # Define a velocidade do som (sound_speed) dependendo do número da imagem (acq).
    # Cada aquisição usa um valor ligeiramente diferente informado pelo dataset.
    if acq == 1:
        sound_speed = 1580
    elif acq == 2:
        sound_speed = 1583
        ...
        # Valor padrão quando a aquisição não está na lista acima
        sound_speed = 1581

    # Carrega os sinais de RF (canal I) do arquivo
    idata = xp.array(arquivo["channel_data"], dtype="float32")

    # Calcula o canal Q aplicando a transformada de Hilbert no eixo do tempo
    qdata = xp.imag(hilbert_xp(idata, axis=-1))

    # Carrega os ângulos de transmissão (ondas planas)
    ...
    # Velocidade do som usada na reconstrução (vem do bloco anterior)
    c = sound_speed

    # time_zero zerado (sem correção temporal extra por ângulo)
    time_zero = xp.zeros((len(angles),), dtype="float32")

    # Nenhuma demodulação sendo aplicada
    fdemod = 0


    # Define a posição física dos elementos do transdutor (L11-4v)

    pitch = 0.3e-3              # distância entre elementos (0,3 mm)
    nelems = idata.shape[1]     # número de elementos do transdutor

    # Gera posições em x igualmente espaçadas
    xpos = xp.arange(nelems) * pitch

    # Centraliza a abertura em torno de x = 0
    xpos -= xp.mean(xpos)
    ...
    # Limites laterais da imagem (baseados na posição do transdutor)
    xlims = [ele_pos[0, 0], ele_pos[-1, 0]]

    # Limites de profundidade da imagem (8 mm a 55 mm)
    zlims = [8e-3, 55e-3]
