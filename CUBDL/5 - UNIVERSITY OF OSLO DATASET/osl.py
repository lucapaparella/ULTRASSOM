...
for acq in imagens:
    ...
    # Calcula o canal Q usando a transformada de Hilbert
    qdata = xp.imag(hilbert_xp(idata, axis=-1))

    # Ângulos de transmissão fornecidos pelo dataset (primeira linha)
    ...
    c = sound_speed   # usando valor específico definido antes

    # time_zero vem do arquivo, multiplicado por -1 para alinhar os sinais
    time_zero = -1 * xp.array(arquivo["start_time"], dtype="float32")[0]

    # Sem demodulação
    fdemod = 0

    # Posições dos elementos (arquivo vem como (3,128), .T vira (128,3))
    ele_pos = xp.array(arquivo["element_positions"], dtype="float32").T

    # Centraliza a abertura em x = 0
    ele_pos[:, 0] -= xp.mean(ele_pos[:, 0])

    # Limites laterais da imagem e profundidade padrão
